            ...
            Pro <span className="text-gold">Investment</span>
                ...
                <div className="relative">
                    <h2 className="font-display text-4xl font-semibold text-ink-text mb-4 leading-tight">
                        Start your<br />
                        <span className="text-gold">wealth journey</span>
                    </h2>
                    <p className="text-muted leading-relaxed max-w-sm mb-10">
                        Join thousands of investors already growing their wealth daily with automated ROI and referral income.
                    </p>

                    {/* Plan highlights */}
                    <div className="space-y-4">
                        {[
                            { plan: 'Basic', range: '₹1K – ₹9,999', roi: '1.0% / day' },
                            { plan: 'Silver', range: '₹10K – ₹49,999', roi: '1.5% / day' },
                            { plan: 'Gold', range: '₹50K – ₹1,99,999', roi: '2.0% / day', popular: true },
                            { plan: 'Platinum', range: '₹2L+', roi: '2.5% / day' },
                        ].map((p) => (
                            <div key={p.plan} className={`flex items-center justify-between p-3 rounded-xl border ${p.popular ? 'bg-gold/10 border-gold/30' : 'bg-surface-raised border-white/5'}`}>
                                <div>
                                    <span className={`text-sm font-medium ${p.popular ? 'text-gold' : 'text-ink-text'}`}>{p.plan}</span>
                                    <span className="text-xs text-muted ml-2">{p.range}</span>
                                </div>
                                <span className={`text-sm font-mono font-semibold ${p.popular ? 'text-gold' : 'text-emerald'}`}>{p.roi}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
              ...
              Pro <span className="text-gold">Investment</span>
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-muted">Start earning daily returns today</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold uppercase tracking-widest text-muted mb-2">
                                Referral Code <span className="text-muted/50 normal-case tracking-normal font-normal">(optional)</span>
                            </label>
                            <input
                                type="text"
                                name="referralCode"
                                ...
                                onChange={handleChange}
                                placeholder="e.g. O0CDF0EW"
                                maxLength={8}
                                className="w-full bg-surface border border-white/10 rounded-xl px-4 py-3 text-ink-text placeholder-muted/50 focus:outline-none focus:border-gold/50 focus:bg-surface-raised transition-all duration-200 text-sm font-mono uppercase"
                            />
                        </div>
